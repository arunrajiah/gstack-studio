...
import { Bot, Copy, Check, Search, X, Wifi, WifiOff, BookOpen, Play } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import { useSkills, useDaemon, useDaemonLogs, useConfig } from '../lib/store'
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-72 flex flex-col border-r border-zinc-200/60 dark:border-zinc-800/60 shrink-0">
        <div className="px-4 pt-4 pb-3 border-b border-zinc-200/60 dark:border-zinc-800/60 space-y-3">
          <div>
            <h1 className="text-base font-semibold text-zinc-900 dark:text-zinc-100">All Agents</h1>
            <p className="text-xs text-zinc-500 mt-0.5">Click any agent to copy its command</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {filtered.length === 0 ? (
            <p className="px-4 py-6 text-xs text-zinc-600 text-center">No agents match</p>
          ) : filtered.map(skill => (
            <SkillRow key={skill.id} skill={skill} hostBin={config.hostBin} onViewDoc={() => setDocSkill(skill)} onNavigateSettings={() => navigate('/settings')} />
          ))}
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-6 py-3 border-b border-zinc-200/60 dark:border-zinc-800/60 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">Live Activity</h2>
            <p className="text-xs text-zinc-500 mt-0.5">Real-time output from the AI browser service</p>
          </div>
          ...
          {logs.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-zinc-600 gap-3">
              ...
              <p className="text-center text-sm">
                {state.running
                  ? 'Waiting for activity…\nRun an agent to see live output here.'
                  : 'Start the AI Browser on the Dashboard to see live activity here.'
                }
              </p>
            </div>
          ) : logs.map((line, i) => (
        ...
        <div className="px-6 py-2 border-t border-zinc-200/60 dark:border-zinc-800/60 text-xs text-zinc-500 dark:text-zinc-600">
          {logs.length} line{logs.length !== 1 ? 's' : ''} · refreshing every 2 s
        </div>
      </div>
    </div>
  )
...
function SkillRow({ skill, hostBin, onViewDoc, onNavigateSettings }: {
  skill: Skill
  hostBin: string
  onViewDoc: () => void
    ...
    <div className="w-full flex items-start gap-3 px-4 py-2.5 hover:bg-zinc-200/50 dark:hover:bg-zinc-800/50 transition-colors group">
      ...
      <div className="flex items-center gap-1 shrink-0 mt-1">
        ...
        <button
          ...
          disabled={running}
          className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded text-zinc-500 hover:text-emerald-400 hover:bg-zinc-300 dark:hover:bg-zinc-700 transition-colors disabled:opacity-50"
        >
