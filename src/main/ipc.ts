...
export function registerIpcHandlers(ipcMain: IpcMain, daemon: GStackDaemon): void {
  ...
  ipcMain.handle('gstack:command', async (_event, command: string, args: string[] = []) => {
    ...
    if (!state.running || !state.port || !state.token) {
  ...
  ipcMain.handle('gstack:skills', async () => {
    ...
    // Check actual filesystem to mark which skills are present
  ...
  // Copy a skill command to clipboard (no gstack changes needed)
...
function configPath(): string {
  return join(homedir(), '.gstack', 'studio-config.json')
}
...
function getConfig(): AppConfig {
  ...
  return { anthropicApiKey: '', gstackPath: '', workspacePath: '', openaiApiKey: '' }
}
